...
function card(item){const img=item.image?`<img loading="lazy" src="${item.image}" alt="" class="h-full w-full object-cover">`:`<div class="text-xs text-neutral-400">No image</div>`;
const rarity=item.rarity!==null?`<span class="inline-flex items-center rounded-md border border-neutral-700/70 bg-neutral-800/70 px-2.5 py-1 text-xs font-medium text-neutral-200">Rarity: ${fmt(item.rarity)}</span>`:"";
const price=item.price!==null?`<span class="inline-flex items-center rounded-md border border-neutral-700/70 bg-neutral-800/70 px-2.5 py-1 text-xs font-medium text-neutral-200">Price: ${fmt(item.price)}</span>`:"";
const purch=item.purchasable!==null?`<span class="inline-flex items-center rounded-md border border-neutral-700/70 bg-neutral-800/70 px-2.5 py-1 text-xs font-medium text-neutral-200">Purchasable: ${item.purchasable?"Yes":"No"}</span>`:"";
const type=`<span class="inline-flex items-center rounded-md border border-neutral-500/60 bg-neutral-800/80 px-2.5 py-1 text-xs font-medium text-white">${item.kind}</span>`;
const pieceType=item.pieceType?`<span class="inline-flex items-center rounded-md border border-neutral-700/70 bg-neutral-800/70 px-2.5 py-1 text-xs font-medium text-neutral-200">${item.pieceType}</span>`:"";
const creator=`<div class="text-xs text-neutral-400">Creator: ${fmt(item.creatorName)}</div>`;
const uuid=`<div class="text-xs text-neutral-500">UUID: ${fmt(item.uuid)}</div>`;
return `<div class="group flex h-full flex-col overflow-hidden rounded-lg border border-neutral-800/70 bg-neutral-900/60 shadow-sm shadow-black/30 transition hover:-translate-y-1 hover:border-neutral-500/70"><div class="flex aspect-[16/10] items-center justify-center bg-neutral-950/60">${img}</div><div class="flex flex-1 flex-col gap-2.5 p-3.5"><div class="text-sm font-semibold text-neutral-100">${fmt(item.title)}</div><div class="flex flex-wrap gap-2">${type}${pieceType}${rarity}${price}${purch}</div>${creator}${uuid}</div></div>`;}
function render(){el("count").textContent=`${state.filtered.length.toLocaleString()} items`;const limit=2000;const html=state.filtered.slice(0,limit).map(card).join("");el("grid").innerHTML=html||`<div class="rounded-lg border border-dashed border-neutral-700/70 bg-neutral-900/40 p-5 text-sm text-neutral-400">No results.</div>`;if(state.filtered.length>limit){el("grid").insertAdjacentHTML("beforeend",`<div class="rounded-lg border border-dashed border-neutral-700/70 bg-neutral-900/40 p-5 text-sm text-neutral-400">Showing first ${limit.toLocaleString()} results. Refine filters to see more.</div>`);}}
...
init().catch(err=>{el("grid").innerHTML=`<div class="rounded-lg border border-dashed border-rose-500/40 bg-rose-500/10 p-5 text-sm text-rose-200">Failed to load data: ${err.message}</div>`;});
