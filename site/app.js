...
const rarity=item.rarity!==null?`<span class="inline-flex items-center rounded-full border border-neutral-700/70 bg-neutral-800/70 px-2.5 py-1 text-xs font-medium text-neutral-200">Rarity: ${fmt(item.rarity)}</span>`:"";
const price=item.price!==null?`<span class="inline-flex items-center rounded-full border border-neutral-700/70 bg-neutral-800/70 px-2.5 py-1 text-xs font-medium text-neutral-200">Price: ${fmt(item.price)}</span>`:"";
const purch=item.purchasable!==null?`<span class="inline-flex items-center rounded-full border border-neutral-700/70 bg-neutral-800/70 px-2.5 py-1 text-xs font-medium text-neutral-200">Purchasable: ${item.purchasable?"Yes":"No"}</span>`:"";
const type=`<span class="inline-flex items-center rounded-full border border-neutral-500/60 bg-neutral-800/80 px-2.5 py-1 text-xs font-medium text-white">${item.kind}</span>`;
const pieceType=item.pieceType?`<span class="inline-flex items-center rounded-full border border-neutral-700/70 bg-neutral-800/70 px-2.5 py-1 text-xs font-medium text-neutral-200">${item.pieceType}</span>`:"";
...
return `<div class="group flex h-full flex-col overflow-hidden rounded-2xl border border-neutral-800/70 bg-neutral-900/60 shadow-md shadow-black/30 transition hover:-translate-y-1 hover:border-neutral-500/70"><div class="flex aspect-[16/10] items-center justify-center bg-neutral-950/60">${img}</div><div class="flex flex-1 flex-col gap-3 p-4"><div class="text-sm font-semibold text-neutral-100">${fmt(item.title)}</div><div class="flex flex-wrap gap-2">${type}${pieceType}${rarity}${price}${purch}</div>${creator}${uuid}</div></div>`;}
function render(){el("count").textContent=`${state.filtered.length.toLocaleString()} items`;const limit=2000;const html=state.filtered.slice(0,limit).map(card).join("");el("grid").innerHTML=html||`<div class="rounded-2xl border border-dashed border-neutral-700/70 bg-neutral-900/40 p-6 text-sm text-neutral-400">No results.</div>`;if(state.filtered.length>limit){el("grid").insertAdjacentHTML("beforeend",`<div class="rounded-2xl border border-dashed border-neutral-700/70 bg-neutral-900/40 p-6 text-sm text-neutral-400">Showing first ${limit.toLocaleString()} results. Refine filters to see more.</div>`);}}
